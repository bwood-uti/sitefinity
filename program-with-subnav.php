<?php
    
?>

<!DOCTYPE html>
<html lang="en">
    <head>
        <!-- Meta -->
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
        <meta name="description" content="">
        <meta name="author" content="">

        <title>Universal Technical Institute</title>

        <?php include("styles.php") ?>
    </head>
    <body id="page-top" data-spy="scroll" data-target=".subnav">
       <div class="container-full">
	       
        <?php include("navbar.php") ?>


        <main>
            <!-- ============================================================= SECTION 
	            
	            hiding the hero image for now.
	            
            <section id="hero">
                <div id="full-width-slider" class="royalSlider heroSlider rsMinW">

                    <div class="rsContent">
                        <img class="rsImg" src="http://www.uti.edu/~/media/images/uti/sliders/programs/auto/hands-on-02.ashx" alt="">

                    </div>

                </div>

                <a href="#program-overview" class="page-scroll circle">
                    <i class="fa fa-chevron-down"></i>
                </a>
            </section>
            ============================================================= -->
            <section id="sub-header" class="sub-header dark-bg">

                <div class="container inner-top-m inner-bottom-xs inner-left-xs inner-right-xs">
                    <div class="row">
                        <div class="col-md-12">
                            <h1 class="no-bottom-margin text-left">Automotive Technician Training</h1>
                        </div>
                    </div>
                </div>
            </section>
            
            
            <!-- ============================================================= NAVIGATION ============================================================= -->
            <nav class="navbar subnav navbar-default" role="navigation">
                <div class="container-fluid">
                    <div class="collapse navbar-collapse navbar-ex1-collapse">
                        <ul class="nav navbar-nav">
                            <!-- Hidden li included to remove active class from about link when scrolled up past about section -->
                            <li class="hidden">
                                <a class="page-scroll" href="#page-top"></a>
                            </li>
                            <li><a href="#program-overview" class="page-scroll">Program Overview</a></li>
                            <li><a href="#power-and-performance" class="page-scroll">Roush Yates Power & Performance</a></li>

                            <li><a href="#msat" class="page-scroll">Manufacturer Training</a></li>

                            <li><a href="#resources" class="page-scroll">Resources</a></li>
                            <li><a href="#campus-locations" class="page-scroll">Campus Locations</a></li>
                        </ul>
                    </div>
                    <!-- /.navbar-collapse -->
                </div>
                <!-- /.container -->
            </nav>
            <!-- ============================================================= NAVIGATION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="program-overview">

                <div class="container inner-top-sm inner-bottom-sm">
                    <h3>Program Overview</h3>
                    <div class="row">
                        <div class="col-md-8 col-sm-9">
                            <p>
                            In this 51-week* program, you'll learn how to diagnose,
                            maintain and repair domestic and foreign automobiles.
                            You will also find out how to troubleshoot problems of
                            all kinds, using the latest engine analyzers, handheld
                            scanners and other computerized diagnostic equipment.
                            </p>
                            <p>

                            You'll learn everything from basic engine systems to
                            computerized fuel injection, anti-lock brakes, passenger
                            restraint systems, computerized engine controls and much more.
                            </p>

                        </div><!-- /.col -->
                        <div class="col-md-4"><img src="http://placehold.it/500x300&text=Video" />Auto Program Overview</div>
                    </div><!-- /.row -->
                    <!-- /.col -->
                </div>

                <div id="course-list" class="uti-blue-bg">
                    <div class="container inner-top-sm inner-bottom-sm">
                        <div class="row">
                            <h1>
                        YOUR COURSES WILL INCLUDE
                            </h1>
                            <div class="col-md-4">
                                <ul class="fa-ul">
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Automotive Engines &amp; Repair</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Professional Service Writing</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Automotive Power Trains</strong></li>
                                </ul>
                            </div>
                            <div class="col-md-4">
                                <ul class="fa-ul">
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Brakes</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Electronic Technology</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Climate Control</strong></li>
                                </ul>
                            </div>
                            <div class="col-md-4">
                                <ul class="fa-ul">
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Driveability &amp; Emissions</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong>Advanced Diagnostic Systems</strong></li>
                                    <li><em class="fa-li fa fa-angle-double-right whiteText"></em><strong><a href="#power-and-performance" class="scrollTo">Power &amp; Performance</a>**</strong></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <section class="light-bg">

                <div class="container inner-top-sm inner-bottom-sm ">
                    <div class="row">
                        <div class="col-md-6">
                            <p>
                            UTI’s Automotive Technology program is Master Certified by the National Automotive
                            Technicians Education Foundation (NATEF), a division of the National Institute for
                            Automotive Service Excellence (ASE).
                            </p>

                            <p>
                            Graduates of UTI’s Automotive Technician Training program are well prepared to complete
                            ASE examinations and can substitute their training for one year of the two years of work
                            experience required to become ASE Certified.
                            </p>
                        </div>
                        <div class="col-md-6"><img src="http://www.uti.edu/~/media/Images/UTI/Programs%20Images/ASE-Logo-560px.ashx" /></div>
                    </div>
                </div><!-- /.container -->
            </section>
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="power-and-performance">

                <div class="container inner-top-sm inner-bottom-sm ">
                    <h3>Roush Yates Power & Performance OVERVIEW</h3>
                    <div class="row">
                        <div class="col-md-8 ">

                            <h4>Want to bring on the horsepower?</h4>

                            <p>
                             As part of UTI’s Automotive
                            Technician Training program, you can learn how to rev up the
                            speed and modify cars off the lot with two highly popular courses.
                            </p>
                            <p>
                            UTI’s 9-week Roush Yates Power & Performance courses deliver special training
                            in engine/ vehicle modification, including blueprinting and design,
                            as well as dyno testing and verification.
                            </p>

                        </div><!-- /.col -->
                        <div class="col-md-4">
                            <img src="http://placehold.it/500x300&text=Video" alt="Video" />Hear From Jesse Campbell – Roush Yates Power & Performance Instructor
                        </div>
                    </div><!-- /.row -->
                    <div class="row inner-top-md">
                        <div class="col-md-4">
                            <img src="http://www.uti.edu/~/media/images/uti/misc%20images/roushyateslogo.ashx" alt="Image" />
                        </div>
                        <div class="col-md-8 ">


                            <p>
                            UTI brings you high-power training. You’ll learn how to get the
                            “biggest bang for the buck” and to optimize performance through modification.
                            </p>
                            <p>
                            The Roush Yates Power & Performance training program meets the needs of the true performance
                            enthusiast, covering everything from converters and camshafts to cylinder heads
                            and nitrous oxide. Performance modifications will be put to the test on a chassis
                            dyno and engine dyno.
                            </p>

                        </div><!-- /.col -->
                    </div><!-- /.row -->
                    <div class="row inner-top-md">
                        <div class="col-md-4">
                            <img src="http://www.uti.edu/~/media/images/uti/programs%20images/power-performace-img-3.ashx" alt="Image" />
                        </div>
                        <div class="col-md-8 ">


                            <p>
                            UTI will give you all the “tuning” skills you’ll need to perform
                            vehicle modifications while keeping your specs and emissions strictly
                            street legal. UTI brings you up to speed in one of the fastest growing
                            sectors in automotive technology: the personalization and performance enhancements of sport compact cars.
                            </p>
                            <p>
                            You’ll get the low-down on power adders — super chargers, turbochargers,
                            cold air induction, nitrous, ignition and exhaust upgrades — and you’ll
                            figure out how they perform during dyno testing.
                            </p>

                        </div><!-- /.col -->
                    </div>

                </div><!-- /.container -->
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="cta-one" class="uti-blue-bg">
                <div class="container inner-xs">
                    <div class="row">
                        <div class="col-sm-10 center-block text-center">
                            <h1 class="single-block">
                            Ready to change your life yet?
                                <a href="https://wrapbootstrap.com/theme/reen-made-for-designers-portfolio-WB00PN23G" target="_blank" class="btn btn-white btn-large black">Take the Next Step</a>
                            </h1>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container -->
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="msat" class="light-bg">
                <div class="container inner-top-sm inner-bottom-sm">
                    <h3>MANUFACTURER-SPECIFIC ADVANCED TRAINING (MSAT)</h3>
                    <h4>The best way to get a job faster, with better pay, and to become more attractive to potential employers.</h4>
                    <div class="row">

                        <div class="col-md-8 col-sm-9">
                            <p>
                            If you want to be best positioned for a career you'll love, UTI's MSAT programs are the way to go. Employers
                            want to hire technicians who are ready to work, and UTI's factory-authorized training ensures that you train
                            on the latest vehicles, tools and equipment supplied by the world's best manufacturers.
                            </p>
                            <p>     On average, graduates who have completed an MSAT program land a job more quickly and bring home higher starting pay.*</p>

                        </div><!-- /.col -->
                        <div class="col-md-4"><img src="http://placehold.it/500x300&text=Video" />Auto Program Overview</div>
                    </div><!-- /.row -->
                </div><!-- /.container -->
                <section id="partners">

                <div class="container inner-top-sm inner-bottom-sm">
                    <h3 class="center-block text-center">Manufacturer Training</h3>
                    <div class="row">
                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/bmw-auto.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>BMW</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>

                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/ford1.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Ford</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>


                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Partners/honda-auto.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Honda</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>



                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/nissan-alt.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Nissan</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>


                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/toyota-alt.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Toyota</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>



                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/mercedes.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Mercedes</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>



                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped Content Thumbs/Partners/mini.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Mini</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>



                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/porsche.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Porsche</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>

                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/volvo-auto.ashxx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>Volvo</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>

                        <div class="col-sm-3">
                            <figure>
                                <a href="partner.php"><img src="http://www.uti.edu/~/media/Images/UTI/Grouped%20Content%20Thumbs/Partners/gm-chevy.ashx" alt="Partner Name"></a>
                                <figcaption class="bordered no-top-border">
                                    <div class="info text-left">
                                        <h5>GM/Chevy</h5>
                                        <p><a href="partner.php">Learn More</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div>


                    </div><!-- /.row -->
                </div>

            </section>
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="resources" class="tint-bg">
                <div class="container inner-top-sm inner-bottom-sm">

                    <div class="row">
                        <div class="col-md-12 col-sm-9 center-block text-center">
                            <header>
                                <div class="divider-1"></div>
                                <h3>RESOURCES</h3>
                                <div class="divider-1"></div>
                            </header>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                    <div class="row inner-top-sm">

                        <div class="col-md-1">
                            <div class="icon pull-right">
                                <i class="icon-money icn"></i>
                            </div><!-- /.icon -->
                        </div><!-- /.col -->
                        <div class="col-md-3 inner-bottom-xs">
                            <h2>Financial Aid</h2>
                            <p class="text-small"><a href="#">Learn more about what financial aid Can do for you. </a> </p>

                        </div><!-- /.col -->
                        <div class="col-md-1">
                            <div class="icon pull-right">
                                <i class="icon-magic icn"></i>
                            </div><!-- /.icon -->
                        </div><!-- /.col -->
                        <div class="col-md-3 inner-bottom-xs">
                            <h2>Scholarships</h2>
                            <p class="text-small"><a href="#">Search and apply, find the best scholarships available to you.</a> </p>

                        </div><!-- /.col -->
                        <div class="col-md-1">
                            <div class="icon pull-right">
                                <i class="icon-book-open icn"></i>
                            </div><!-- /.icon -->
                        </div><!-- /.col -->
                        <div class="col-md-3 inner-bottom-xs">
                            <h2>Catalogs</h2>
                            <p class="text-small"><a href="#">Download Course Catalogs</a></p>

                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container -->
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <!-- ============================================================= SECTION ============================================================= -->
            <section id="campus-locations" class="light-bg">
                <div class="container container inner-top-sm inner-bottom-xs">
                    <div class="row">
                        <div class="col-md-8 col-sm-9 center-block text-center">

                            <h3 class="no-bottom-margin">Available @ These Campuses</h3>

                        </div><!-- /.col -->
                    </div><!-- /.row -->
                    <div class="row inner-top-sm">


                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Avondale, AZ</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Exton, PA</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Houston, TX</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Dallas/Ft. Worth, TX</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Lisle, IL</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Mooresville, NC</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Norwood, MA</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Orlando, FL</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Rancho Cucamonga, CA</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                        <div class="col-sm-4">
                            <figure>
                                <img src="assets/images/art/work01.jpg" alt="">
                                <figcaption class="bordered no-top-border">
                                    <div class="info">
                                        <h5>Sacramento, CA</h5>
                                        <p> <a href="#">Check Out the Campus</a></p>
                                    </div><!-- /.info -->
                                </figcaption>
                            </figure>

                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container -->
            </section>
            <!-- ============================================================= SECTION END ============================================================= -->
            <?php include("cta-bottom.php") ?>

            <!-- ============================================================= SECTION – TAKE THE NEXT STEP : END ============================================================= -->
        </main>


        <?php include("social-media.php") ?>

        <?php include("footer.php") ?>
        <?php include("javascript.php") ?>


       </div> <!-- end container-full -->
    </body>


</html>
